...
<body>
    <div class="container">
        ...
        <div class="game-wrapper">
            ...
            <div class="buttons-container">
                <a href="../../plataforma.php" class="btn-secondary">← Tornar</a>
            </div>
  ...
  const baseIP = <?= json_encode($BASE_IP) ?>;
    ...
    console.log("Cargando configuración del nivel actual...");
    ...
    try {
        ...
        console.log(`Respuesta API Nivel ${currentLevel}:`, data);

        nivelesConfig[currentLevel] = {
            velocidad: data.velocidad || (3 + currentLevel * 0.5),
            obstaculos: data.obstaculos || (8 + currentLevel * 2),
            puntuacio_minima: data.puntuacio_minima || (currentLevel * 10),
            nom: data.nom || `Nivel ${currentLevel}`
        };

    } catch (error) {
        console.error(`Error cargando nivel ${currentLevel} desde API:`, error);
        nivelesConfig[currentLevel] = {
            velocidad: 3 + currentLevel * 0.5,
            obstaculos: 8 + currentLevel * 2,
            puntuacio_minima: currentLevel * 10,
            nom: `Nivel ${currentLevel}`
        };
    }

    console.log("Configuración cargada:", nivelesConfig);
    loadLevelConfig();
}
        ...
        function loadLevelConfig() {
            ...
            if (config) {
                ...
                minScoreRequired = config.puntuacio_minima;
        ...
        document.addEventListener('keydown', (e) => {
            if (e.code === 'Space') {
                ...
                if (!gameStarted && !gameOver && !levelComplete) {
                    startGame();
                } else if (!gameOver && !levelComplete && gameStarted) {
...
async function nextLevel() {
    ...
    // 🔹 NUEVO: mostramos el overlay mientras carga la configuración del nuevo nivel
    document.getElementById('levelCompleteOverlay').classList.remove('active');
    document.getElementById('startOverlay').classList.add('active');
    document.getElementById('levelInfo').textContent = 'Carregant nivell...';

    // 🔹 NUEVO: recargamos configuración desde la API antes de iniciar el nivel
    await cargarConfiguracionAPI();

    // 🔹 Mantenido: iniciar el juego una vez cargada la configuración
    startGame();
}
        ...
        function updatePipes() {
            ...
            for (let i = pipes.length - 1; i >= 0; i--) {
                ...
                if (!pipes[i].passed && pipes[i].x + pipeWidth < bird.x) {
                    ...
                    // Verificar si se cumplieron las condiciones de victoria
                    if (pipesPassed >= pipesPerLevel && score >= minScoreRequired) {
                        endGame();
                        return;
                    }
                }
        ...
        function endGame() {
            ...
            const minScore = minScoreRequired || 0;
            ...
            if (pipesPassed >= pipesPerLevel && score >= minScore) {
                ...
                if (currentLevel < 5) {
                    ...
                    document.getElementById('nextLevelBtn').textContent = '➡ Següent Nivell';
                } else {
            ...
            fetch(url)
                ...
                .then(text => {
                    try {
                        ...
                        console.log('Respuesta de servidor:', text);
                    }
                })
                .catch(error => {
